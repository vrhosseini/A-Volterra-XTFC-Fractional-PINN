...
optimizer3 = torch.optim.LBFGS(modelv1.parameters(), lr=1,history_size=150, line_search_fn="strong_wolfe", tolerance_grad=1e-8, tolerance_change=1e-128)
optimizer4 = torch.optim.LBFGS(modelv2.parameters(), lr=1,history_size=150, line_search_fn="strong_wolfe", tolerance_grad=1e-8, tolerance_change=1e-128)
    ...
    for epoch in range(epochs):
        # for (x_batch1, t_batch1), (x_batch2, t_batch2) in zip(dataloader1, dataloader2):
            def closure():
                ...
                loss =loss_PDE1  +loss_PDE2+ loss_BC1+ loss_BC2+loss_IC1+loss_IC2+(loss_IFC+loss_IFCN)
