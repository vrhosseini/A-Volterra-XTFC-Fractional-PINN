# -*- coding: utf-8 -*-
"""
Created on Fri Oct 25 17:49:19 2024
...
a=3*eps/r
#29.242508064
I =  3.6553135080000008
ampl= 3.6553135080000008
...
#  			return x.mul_(torch.sigmoid(x)) 
		def forward(self, x):
 			return torch.tanh(x)  # SiLU (Swish) activation function
...
def LossPDE2(x, t):
    ...
    f_source = U2_t - (Epsl_cat**0.5)*U2_xx+(((1-t_0)*(ampl/A)*R**2)/(D2*1000*Epsl_cat*L*F))
...
def Loss_IFC( xface,tface):
    ...
    f1_source = -Ur+Ul
    u_avrag=(Ur+Ul)/2
    ux_avrag=(Ur_x+Ul_x)/2
    f2_source =-Epsl_cat**1.5* Ur_x+Epsl_sep**1.5*Ul_x
    # f2_source =- Ur_x+Ul_x
    ...
    lost_avara1_x=loss_f1(ux_avrag  , Ur_x)
    lost_avara2_x=loss_f1(ux_avrag  , Ul_x)
    return loss_IFC,loss_IFCN,lost_avara1,lost_avara2,lost_avara1_x,lost_avara2_x
...
epochs  =200
# Domain bounds
lb1 = np.array([0, 0.1]) #lower bound
ub1 = np.array([0.21428571428571425, T])  #upper bound
lb2 = np.array([0.21428571428571429, 0.1]) #lower bound
ub2 = np.array([1, T])  #upper bound

N_col_point1=800
N_col_point2=2500
numInitialConditionPoints=25
numBoundaryConditionPoints1 = 25
numBoundaryConditionPoints2 =25
numinterfaceConditionPoints=25
...
optimizer3 = torch.optim.LBFGS(modelv1.parameters(), lr=1,history_size=150, line_search_fn="strong_wolfe", tolerance_grad=1e-9, tolerance_change=1e-32)
optimizer4 = torch.optim.LBFGS(modelv2.parameters(), lr=1,history_size=150, line_search_fn="strong_wolfe", tolerance_grad=1e-9, tolerance_change=1e-32)

# optimizer3 =torch.optim.LBFGS(modelv1.parameters(), lr= 1, max_iter= 10000, max_eval= 5000, history_size= 50, line_search_fn= 'strong_wolfe', tolerance_change= 0.5* np.finfo(float).eps, tolerance_grad= 1e-05)
# optimizer4 =torch.optim.LBFGS(modelv1.parameters(), lr= 1, max_iter= 10000, max_eval= 5000, history_size= 50, line_search_fn= 'strong_wolfe', tolerance_change= 0.5* np.finfo(float).eps, tolerance_grad= 1e-05)
    ...
    for epoch in range(epochs):
        # for (x_batch1, t_batch1), (x_batch2, t_batch2) in zip(dataloader1, dataloader2):
            def closure():
                ...
                loss_IFC,loss_IFCN,lost_avara1,lost_avara2,lost_avara1_x,lost_avara2_x=Loss_IFC( xface,tface)
            
                loss =loss_PDE1  +loss_PDE2+ loss_BC1+ loss_BC2+loss_IC1+loss_IC2+(loss_IFC+5*loss_IFCN)
