...
def Loss_IFC( xface,tface):
    ...
    f2_source =-Epsl_sep**1.5* Ur_x+Epsl_cat**1.5*Ul_x
    # f2_source =- Ur_x+Ul_x
...
optimizer3 = torch.optim.LBFGS(modelv1.parameters(), lr=1,history_size=150, line_search_fn="strong_wolfe", tolerance_grad=1e-32, tolerance_change=1e-32)
optimizer4 = torch.optim.LBFGS(modelv2.parameters(), lr=1,history_size=150, line_search_fn="strong_wolfe", tolerance_grad=1e-128, tolerance_change=1e-128)
    ...
    for epoch in range(epochs):
        # for (x_batch1, t_batch1), (x_batch2, t_batch2) in zip(dataloader1, dataloader2):
            def closure():
                ...
                loss =loss_PDE1  +loss_PDE2+ loss_BC1+ loss_BC2+loss_IC1+loss_IC2+(loss_IFC+10*loss_IFCN)+lost_avara1+lost_avara2+lost_avara1_x+10*lost_avara2_x
